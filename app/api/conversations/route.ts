...
// POST /api/conversations - client starts a chat (requires login)
export async function POST(request: Request) {
    try {
      ...
      // Prevent trainers from messaging themselves
      const { data: senderProfile } = await supabase
            .from('trainer_profiles')
            .select('id')
            .eq('user_id', user.id)
            .single()
          if (senderProfile?.id === trainer_id) {
      ...
      if (!conv) {
              // Check trainer's plan and lead count to determine claim status
            ...
            // Count how many leads this trainer has already claimed
            ...
            // Pro/Elite trainers auto-claim. Free trainers under limit also auto-claim.
      ...
      // Only allow sending messages if conversation is claimed
