...
export async function POST(request: Request) {
  try {
    ...
    const { trainer_id, initial_message, message: messageField } = body
    const messageText = initial_message || messageField
    if (!trainer_id || !messageText) return NextResponse.json({ error: 'Missing fields' }, { status: 400 })
