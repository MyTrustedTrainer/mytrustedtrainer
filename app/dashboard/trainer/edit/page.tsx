...
function calcCompletion(form: any, specialties: string[], hasAvatar: boolean) {
  ...
  const avatarBonus = hasAvatar ? 1 : 0
  return Math.round(((filled + hasSpecialties + avatarBonus) / (fields.length + 2)) * 100)
}
...
export default function TrainerEditProfile() {
    ...
    async function load() {
      ...
      if (p) {
        ...
        setForm({
          ...
          instagram: p.instagram || '', facebook: p.facebook || '', youtube: p.youtube || '', tiktok: p.tiktok || '',
          zip: p.zip || '', price_per_session: p.price_per_session ? String(p.price_per_session) : '',
          training_format: p.training_format || 'both', availability_days: p.availability_days || [], availability_note: p.availability_note || '',
        })
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) return
      const pct = calcCompletion(form, selectedSpecialties, !!avatarUrl)
      const profileData = {
        full_name: form.full_name, tagline: form.tagline, bio: form.bio, phone: form.phone, email: form.email,
        website: form.website, instagram: form.instagram, facebook: form.facebook, youtube: form.youtube, tiktok: form.tiktok,
        zip: form.zip, price_per_session: form.price_per_session ? parseFloat(form.price_per_session) : null,
        training_format: form.training_format, availability_days: form.availability_days, availability_note: form.availability_note,
      ...
      if (profile) {
        ...
        if (error) throw error
        setProfile(newProfile); profileId = newProfile.id
      }
      if (profileId) {
    ...
    } catch (err: any) { setMessage('Error saving: ' + err.message) }
    setSaving(false)
  }

  const completionPct = calcCompletion(form, selectedSpecialties, !!avatarUrl)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-1 bg-gray-100 rounded-xl p-1 mb-6 overflow-x-auto">
          {tabs.map(tab => (
            <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={"flex-1 min-w-max py-2 px-3 rounded-lg text-sm font-medium " + (activeTab === tab.id ? 'bg-white shadow text-[#03243F]' : 'text-gray-500')}>
              {tab.label}
            </button>
          ))}
        </div>
          ...
          {activeTab === 'basics' && (
            <div className="bg-white rounded-2xl shadow-sm p-6 space-y-6">
              ...
              <div className="flex items-center gap-6 pb-6 border-b border-gray-100">
                {user && (
                  <AvatarUpload uid={user.id} url={avatarUrl} name={form.full_name || 'Trainer'} size={96} table="trainer_profiles" onUpload={(url) => setAvatarUrl(url)} />
                )}
                <div>
                  <p className="font-semibold text-[#03243F]">Profile Photo</p>
                  <p className="text-sm text-gray-500">Your photo appears on your public profile and in search results.</p>
              ...
              <div>
                ...
                <input type="text" value={form.full_name} onChange={e => setForm({...form, full_name: e.target.value})} required className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Tagline</label>
                <input type="text" value={form.tagline} onChange={e => setForm({...form, tagline: e.target.value})} placeholder="e.g. NASM Certified | Weight Loss Specialist" className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Bio</label>
                <textarea value={form.bio} onChange={e => setForm({...form, bio: e.target.value})} rows={5} placeholder="Tell potential clients about your experience..." className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]" />
              </div>
            </div>
          )}
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {([{label:'Email *',field:'email',type:'email'},{label:'Phone',field:'phone',type:'tel'},{label:'Website',field:'website',type:'url'},{label:'ZIP Code',field:'zip',type:'text'}] as {label:string,field:string,type:string}[]).map(({label,field,type}) => (
                  <div key={field}>
                    ...
                    <input type={type} value={(form as any)[field]} onChange={e => setForm({...form,[field]:e.target.value})} required={field==='email'} className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]" />
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {([{label:'Instagram',field:'instagram',ph:'@username'},{label:'Facebook',field:'facebook',ph:'Profile URL'},{label:'YouTube',field:'youtube',ph:'Channel URL'},{label:'TikTok',field:'tiktok',ph:'@username'}] as {label:string,field:string,ph:string}[]).map(({label,field,ph}) => (
                  <div key={field}>
                    <label className="block text-sm font-medium text-gray-700 mb-1">{label}</label>
                    <input type="text" value={(form as any)[field]} onChange={e => setForm({...form,[field]:e.target.value})} placeholder={ph} className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" value={form.price_per_session} onChange={e => setForm({...form, price_per_session: e.target.value})} placeholder="65" min="0" step="5" className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Training Format</label>
                  <select value={form.training_format} onChange={e => setForm({...form, training_format: e.target.value})} className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {AVAILABILITY_DAYS.map(day => (
                  <button type="button" key={day} onClick={() => setForm(f => ({...f, availability_days: f.availability_days.includes(day) ? f.availability_days.filter(d => d !== day) : [...f.availability_days, day]}))}
              ...
              <div>
                ...
                <textarea value={form.availability_note} onChange={e => setForm({...form, availability_note: e.target.value})} rows={3} placeholder="e.g. Early mornings preferred..." className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]" />
              </div>
            </div>
          )}
