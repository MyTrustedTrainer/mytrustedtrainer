                ...
                <div className="p-5">
                        <div className="flex items-start gap-4 mb-4">
                                  ...
                                  <div className="flex-1">
                                              ...
                                              <p className="text-xs text-gray-400 mt-1">Wants to start a live chat</p>p>
                                  </div>div>
                        </div>div>
                    {needsPayment ? (
                        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4">
                                    <p className="text-xs text-amber-800 font-semibold mb-1">Free leads used up</p>p>
                                    <p className="text-xs text-amber-700">Accept this chat for $15 one-time, or upgrade to Pro for unlimited leads.</p>p>
                        </div>div>
                      ) : (
                        ...
                        <div className="flex gap-2">
                            ...
                            {!needsPayment && (
                          <a href="/for-trainers" className="px-3 py-2.5 bg-blue-100 text-blue-700 rounded-xl text-xs font-semibold hover:bg-blue-200 transition-colors text-center">Go Pro</a>a>
                                  )}
            ...
            const unread = conv.messages?.filter((m: any) => m.sender_type === 'client' && !m.is_read).length || 0
                  return (
                          <a href={'/dashboard/trainer/messages/' + conv.id}
                              ...
                              {lastMsg && <p className="text-sm text-gray-500 truncate">{lastMsg.sender_type === 'trainer' ? 'You: ' : ''}{lastMsg.body}</p>p>}
                          </a>a>
                        )
                      }
                                                    ...
                                                    <div className="relative overflow-hidden rounded-2xl border-2 border-amber-300 bg-gradient-to-r from-amber-50 to-orange-50">
                                                            <div className="p-5 filter blur-sm select-none pointer-events-none">
                                                                      <div className="flex items-start gap-4">
                                                                                  ...
                                                                                  <div className="flex-1"><div className="h-5 bg-gray-300 rounded w-32 mb-2" /><div className="h-4 bg-gray-200 rounded w-48" /></div>div>
                                                                      </div>div>
                                                            </div>div>
                                                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-t from-amber-100/90 to-transparent p-4">
                                                                      <div className="bg-white rounded-2xl shadow-lg p-5 text-center max-w-xs w-full border border-amber-200">
                                                                                  ...
                                                                                  <div className="space-y-2">
                                                                                                <button onClick={() => onUnlock(lead.id)} className="w-full bg-amber-500 hover:bg-amber-600 text-white py-2.5 rounded-xl font-semibold text-sm">Unlock for $15</button>button>
                                                                                                <a href="/for-trainers" className="block w-full border-2 border-[#18A96B] text-[#18A96B] py-2.5 rounded-xl font-semibold text-sm text-center hover:bg-[#18A96B] hover:text-white">Upgrade to Pro - $49/mo</a>a>
                                                                                  </div>div>
                                                                      </div>div>
                                                            </div>div>
                                                    </div>div>
                                                        ...
                                                        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                                                            {lead.status === 'new' && (
                                                                    <div className="bg-gradient-to-r from-[#18A96B] to-emerald-400 px-4 py-1.5">
                                                                              <span className="text-white text-xs font-bold tracking-wide">NEW LEAD - {new Date(lead.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>span>
                                                                    </div>div>
                                                              )}
                                                              <div className="p-5">
                                                                      <div className="flex items-start gap-4">
                                                                                ...
                                                                                <div className="flex-1 min-w-0">
                                                                                            <div className="flex items-start justify-between gap-2 mb-1">
                                                                                                          ...
                                                                                                          <span className={"text-xs px-2.5 py-1 rounded-full font-medium " + (lead.status === 'new' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600')}>{lead.status}</span>span>
                                                                                                </div>div>
                                                                                            <p className="text-sm text-gray-600 mb-3">{lead.email}</p>p>
                                                                                                                ...
                                                                                                                const [activeTab, setActiveTab] = useState<'leads'|'chats'>('chats')
                                                                                                                      ...
                                                                                                                      useEffect(() => {
                                                                                                                            ...
                                                                                                                            async function handleClaimConversation(convId: string) {
                                                                                                                                    const res = await fetch('/api/conversations/' + convId + '/claim', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) })
                                                                                                                                            const data = await res.json()
                                                                                                                                                    if (data.success) {
                                                                                                            ...
                                                                                                            <div className="max-w-7xl mx-auto px-4 py-8">
                                                                                                                {!profile ? (
                                                                                                                    <div className="bg-white rounded-2xl shadow-sm p-8 text-center">
                                                                                                                                <h2 className="text-2xl font-bold text-[#03243F] mb-4">Complete Your Profile</h2>h2>
                                                                                                                    ...
                                                                                                                    <>
                                                                                                                                ...
                                                                                                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                                                                                                                                              <div className="bg-white rounded-2xl shadow-sm p-5"><p className="text-xs text-gray-500 font-medium mb-1 uppercase tracking-wide">Profile Views</p>p><p className="text-3xl font-bold text-[#03243F]">{views}</p>p></div>div>
                                                                                                                                              <div className="bg-white rounded-2xl shadow-sm p-5"><p className="text-xs text-gray-500 font-medium mb-1 uppercase tracking-wide">Chats Claimed</p>p><p className="text-3xl font-bold text-[#18A96B]">{claimedCount}</p>p>{isFree && <p className="text-xs text-gray-400">{Math.min(claimedCount, FREE_LEAD_LIMIT)}/{FREE_LEAD_LIMIT} free</p>p>}</div>div>
                                                                                                                                              <div className="bg-white rounded-2xl shadow-sm p-5"><p className="text-xs text-gray-500 font-medium mb-1 uppercase tracking-wide">Avg Rating</p>p><p className="text-3xl font-bold text-[#F4A636]">{profile.trainer_scores?.overall_score?.toFixed(1) || 'N/A'}</p>p></div>div>
                                                                                                                                              <div className="bg-white rounded-2xl shadow-sm p-5"><p className="text-xs text-gray-500 font-medium mb-1 uppercase tracking-wide">Unread Msgs</p>p><p className="text-3xl font-bold text-[#03243F]">{unreadMessages}</p>p></div>div>
                                                                                                                                    </div>div>
                                                                                                                                      ...
                                                                                                                                      <div className="space-y-4">
                                                                                                                                          {pendingConversations.length > 0 && (
                                                                                                                                                            <div>
                                                                                                                                                                                ...
                                                                                                                                                                                <div className="space-y-4">
                                                                                                                                                                                    {pendingConversations.map(conv => <PendingChatCard key={conv.id} conv={conv} onClaim={handleClaimConversation} onIgnore={handleIgnoreConversation} isFree={isFree} claimedCount={claimedCount} />)}
                                                                                                                                                                                    </div>div>
                                                                                                                                                                </div>div>
                                                                                                                                                      )}
                                                                                                                                          {claimedConversations.length > 0 && (
                                                                                                                                                            <div>
                                                                                                                                                                {pendingConversations.length > 0 && <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3 mt-6">Active Chats</h3>h3>}
                                                                                                                                                                                <div className="space-y-3">{claimedConversations.map(conv => <ConversationCard key={conv.id} conv={conv} />)}</div>div>
                                                                                                                                                                </div>div>
                                                                                                                                                      )}
                                                                                                                                          {conversations.length === 0 && (
                                                                                                                                                            <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
                                                                                                                                                                                <div className="text-5xl mb-4">💬</div>div>
                                                                                                                                                                                <h3 className="text-xl font-bold text-[#03243F] mb-2">No chats yet</h3>h3>
                                                                                                                        ...
                                                                                                                        {activeTab === 'leads' && (
                                                                                                                                      <div className="space-y-4">
                                                                                                                                          {leads.length === 0 ? (
                                                                                                                                                            <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
                                                                                                                                                                                <div className="text-5xl mb-4">📬</div>div>
                                                                                                                                                                                <h3 className="text-xl font-bold text-[#03243F] mb-2">No leads yet</h3>h3>
                                                                                                                                                                                <a href="/dashboard/trainer/edit" className="inline-block bg-[#18A96B] text-white px-6 py-3 rounded-xl font-semibold text-sm">Complete Profile</a>a>
                                                                                                                                                                </div>div>
