  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border-2 border-amber-300 overflow-hidden">
      <div className="bg-gradient-to-r from-amber-500 to-orange-400 px-4 py-2 flex items-center gap-2">
        <span className="text-white text-xs font-bold tracking-wide">NEW CHAT REQUEST</span>
        <span className="text-amber-100 text-xs">- {new Date(conv.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
      </div>
      <div className="p-5">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            <h4 className="font-bold text-[#03243F] text-lg">{conv.client_name}</h4>
            <p className="text-sm text-gray-500">{conv.client_email}</p>
            <p className="text-xs text-gray-400 mt-1">Wants to start a live chat</p>
          </div>
        </div>
        {needsPayment ? (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4">
            <p className="text-xs text-amber-800 font-semibold mb-1">Free leads used up</p>
            <p className="text-xs text-amber-700">Accept this chat for $15 one-time, or upgrade to Pro for unlimited leads.</p>
          </div>
        ) : (
          <div className="bg-green-50 border border-green-200 rounded-xl p-3 mb-4">
            <p className="text-xs text-green-800">Free lead ({claimedCount + 1} of {FREE_LEAD_LIMIT}). No cost to accept.</p>
          </div>
        )}
        <div className="flex gap-2">
          <button onClick={handleClaim} disabled={claiming} className="flex-1 bg-[#18A96B] text-white py-2.5 rounded-xl font-semibold text-sm hover:bg-[#15906A] disabled:opacity-50 transition-colors">
            {claiming ? 'Accepting...' : needsPayment ? 'Accept for $15' : 'Accept Chat - Free'}
          </button>
          {!needsPayment && (
            <a href="/for-trainers" className="px-3 py-2.5 bg-blue-100 text-blue-700 rounded-xl text-xs font-semibold hover:bg-blue-200 transition-colors text-center">Go Pro</a>
          )}
          <button onClick={handleIgnore} disabled={ignoring} className="px-3 py-2.5 bg-gray-100 text-gray-500 rounded-xl text-sm hover:bg-gray-200 disabled:opacity-50 transition-colors">
            {ignoring ? '...' : 'Ignore'}
          </button>
        </div>
      </div>
...
function ConversationCard({ conv }: { conv: any }) {
  ...
  return (
    <a href={'/dashboard/trainer/messages/' + conv.id} className={'block bg-white rounded-2xl shadow-sm p-5 hover:shadow-md transition-shadow border-l-4 ' + (unread > 0 ? 'border-[#18A96B]' : 'border-transparent')}>
      ...
      {lastMsg && <p className="text-sm text-gray-500 truncate">{lastMsg.sender_type === 'trainer' ? 'You: ' : ''}{lastMsg.body}</p>}
    </a>
  )
}
...
function LeadCard({ lead, index, isFree, onUnlock }: { lead: any, index: number, isFree: boolean, onUnlock: (id: string) => void }) {
      ...
      <div className="relative overflow-hidden rounded-2xl border-2 border-amber-300 bg-gradient-to-r from-amber-50 to-orange-50">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-t from-amber-100/90 to-transparent p-4">
          <div className="bg-white rounded-2xl shadow-lg p-5 text-center max-w-xs w-full border border-amber-200">
            ...
            <p className="text-sm text-gray-600 mb-4">You have used your 3 free leads. Unlock this one for $15 or upgrade to Pro.</p>
            <div className="space-y-2">
              <button onClick={() => onUnlock(lead.id)} className="w-full bg-amber-500 hover:bg-amber-600 text-white py-2.5 rounded-xl font-semibold text-sm">Unlock for $15</button>
              <a href="/for-trainers" className="block w-full border-2 border-[#18A96B] text-[#18A96B] py-2.5 rounded-xl font-semibold text-sm text-center hover:bg-[#18A96B] hover:text-white">Upgrade to Pro - $49/mo</a>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
      {lead.status === 'new' && (
        <div className="bg-gradient-to-r from-[#18A96B] to-emerald-400 px-4 py-1.5">
          <span className="text-white text-xs font-bold tracking-wide">NEW LEAD - {new Date(lead.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
        </div>
      )}
      <div className="p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-1">
              ...
              <span className={"text-xs px-2.5 py-1 rounded-full font-medium " + (lead.status === 'new' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600')}>{lead.status}</span>
            </div>
            ...
            <a href={'mailto:' + lead.email + '?subject=Re: Your message on MyTrustedTrainer'} className="block text-center bg-[#03243F] text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-[#04305a] transition-colors">
              Reply via Email
            </a>
          </div>
        </div>
...
export default function TrainerDashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setUser(user)
      const { data: profile } = await supabase.from('trainer_profiles').select('*, trainer_scores(*)').eq('user_id', user.id).single()
      setProfile(profile)
      setAvatarUrl(profile?.avatar_url || null)
      if (profile) {
        const { data: leadsData } = await supabase.from('leads').select('*').eq('trainer_id', profile.id).order('created_at', { ascending: false })
        setLeads(leadsData || [])
        const { data: convData } = await supabase.from('conversations').select('*, messages(id, body, sender_type, created_at, is_read)').eq('trainer_id', profile.id).order('last_message_at', { ascending: false })
        setConversations(convData || [])
        const { count } = await supabase.from('profile_views').select('*', { count: 'exact', head: true }).eq('trainer_id', profile.id)
        setViews(count || 0)
      }
      setLoading(false)
    }
  ...
  const unreadMessages = claimedConversations.reduce((acc, c) => acc + (c.messages || []).filter((m: any) => m.sender_type === 'client' && !m.is_read).length, 0)
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {!profile ? (
          <div className="bg-white rounded-2xl shadow-sm p-8 text-center">
            <h2 className="text-2xl font-bold text-[#03243F] mb-4">Complete Your Profile</h2>
          ...
          <>
            <div className="bg-white rounded-2xl shadow-sm p-6 mb-6 flex flex-col md:flex-row items-start md:items-center gap-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  {isFree && <a href="/for-trainers" className="text-sm text-[#18A96B] font-semibold hover:underline">Upgrade to Pro</a>}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm p-5">
                <p className="text-xs text-gray-500 font-medium mb-1 uppercase tracking-wide">Chats Claimed</p>
                <p className="text-3xl font-bold text-[#18A96B]">{claimedCount}</p>
                {isFree && <p className="text-xs text-gray-400">{Math.min(claimedCount, FREE_LEAD_LIMIT)}/{FREE_LEAD_LIMIT} free</p>}
              </div>
              <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            {pendingConversations.length > 0 && (
              <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl p-5 mb-6 flex items-center gap-4 text-white">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-lg">{pendingConversations.length} pending chat request{pendingConversations.length !== 1 ? 's' : ''}!</h3>
                  <p className="text-amber-100 text-sm">Clients are waiting. Accept to start a live chat.</p>
                </div>
              </div>
            )}

            <div className="flex gap-1 bg-gray-100 rounded-xl p-1 mb-6 w-fit">
              <button onClick={() => setActiveTab('chats')} className={"px-5 py-2 rounded-lg text-sm font-semibold transition-colors " + (activeTab === 'chats' ? 'bg-white shadow text-[#03243F]' : 'text-gray-500 hover:text-gray-700')}>
                Chats {(pendingConversations.length + unreadMessages) > 0 && <span className="ml-1.5 bg-[#18A96B] text-white text-xs px-1.5 py-0.5 rounded-full">{pendingConversations.length + unreadMessages}</span>}
              </button>
              <button onClick={() => setActiveTab('leads')} className={"px-5 py-2 rounded-lg text-sm font-semibold transition-colors " + (activeTab === 'leads' ? 'bg-white shadow text-[#03243F]' : 'text-gray-500 hover:text-gray-700')}>
                Leads {newLeadsCount > 0 && <span className="ml-1.5 bg-amber-500 text-white text-xs px-1.5 py-0.5 rounded-full">{newLeadsCount}</span>}
              </button>
            </div>
            ...
            {activeTab === 'chats' && (
              <div className="space-y-4">
                {pendingConversations.length > 0 && (
                  <div>
                    <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Pending Requests</h3>
                    <div className="space-y-4">
                ...
                {claimedConversations.length > 0 && (
                  <div>
                    {pendingConversations.length > 0 && <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3 mt-6">Active Chats</h3>}
                ...
                {conversations.length === 0 && (
                  <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
                    ...
                    <h3 className="text-xl font-bold text-[#03243F] mb-2">No chats yet</h3>
                    <p className="text-gray-500">When clients message you through your profile, they will appear here.</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'leads' && (
              <div className="space-y-4">
                {leads.length === 0 ? (
                  <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
                    <div className="text-5xl mb-4">&#x1F4EC;</div>
                    <h3 className="text-xl font-bold text-[#03243F] mb-2">No leads yet</h3>
                    <a href="/dashboard/trainer/edit" className="inline-block bg-[#18A96B] text-white px-6 py-3 rounded-xl font-semibold text-sm">Complete Profile</a>
                  </div>
                ) : leads.map((lead, i) => <LeadCard key={lead.id} lead={lead} index={i} isFree={isFree} onUnlock={handleUnlockLead} />)}
              </div>
            )}
          </>
        )}
