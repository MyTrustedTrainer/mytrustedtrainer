                          ...
                          <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-4">
                                                ...
                                                <button onClick={async () => { await supabase.auth.signOut(); router.push('/') }}
                                                                className="text-sm text-gray-300 hover:text-white">Sign Out</button>button>
                                    </div>div>
                          </div>div>
                  </header>header>
                                                ...
                                                <form onSubmit={handleSave} className="space-y-4">
                                                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                                              ...
                                                                              <div className="md:col-span-2">
                                                                                                ...
                                                                                                <textarea value={form.fitness_goals}
                                                                                                                      onChange={e => setForm({...form, fitness_goals: e.target.value})}
                                                                                                                      rows={3} placeholder="What are you looking to achieve? (e.g. lose 20 lbs, train for a marathon, build muscle...)"
                            ...
                            <div className="space-y-3">
                              {savedTrainers.map((s: any) => (
                                                <div key={s.id} className="flex items-center justify-between">
                                                                    ...
                                                                    <Link href={`/trainers/${s.trainer_profiles?.slug}`}
                                                                                            className="text-xs text-[#18A96B] hover:underline">View</Link>Link>
                                                </div>div>
                                              ))}
                            </div>div>
                                                )}
