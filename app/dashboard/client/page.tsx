...
export default function ClientDashboard() {
  ...
  const [form, setForm] = useState({
    full_name: '', phone: '', fitness_goals: '', age: '', gender: '', experience_level: '',
  })
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: saved } = await supabase.from('saved_trainers').select('*, trainer_profiles(full_name, slug, tagline, plan)').eq('client_id', user.id)
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h2 className="text-2xl font-bold text-[#03243F] mb-6" style={{fontFamily:'Playfair Display'}}>My Profile</h2>
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" value={form.age} onChange={e => setForm({...form, age: e.target.value})} placeholder="25" min="13" max="100"
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B]" />
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea value={form.fitness_goals} onChange={e => setForm({...form, fitness_goals: e.target.value})} rows={3}
                    placeholder="What are you looking to achieve?" className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#18A96B] resize-none" />
                </div>
              </div>
              ...
              <button type="submit" disabled={saving} className="w-full bg-[#18A96B] text-white py-3 rounded-xl font-semibold hover:bg-[#15906A] disabled:opacity-50 transition-colors">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h3 className="text-lg font-bold text-[#03243F] mb-3" style={{fontFamily:'Playfair Display'}}>Account</h3>
            <p className="text-sm text-gray-600 mb-1">{user?.email}</p>
            <p className="text-xs text-gray-400">Free client account</p>
            <div className="mt-4 pt-4 border-t border-gray-100">
              <Link href="/search" className="block w-full text-center bg-[#03243F] text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-[#04305a] transition-colors">
                Find a Trainer
              </Link>
            </div>
          </div>
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h3 className="text-lg font-bold text-[#03243F] mb-3" style={{fontFamily:'Playfair Display'}}>Saved Trainers</h3>
            {savedTrainers.length === 0 ? (
              <p className="text-sm text-gray-500">No saved trainers yet.</p>
            ) : (
              <div className="space-y-3">
                {savedTrainers.map((s: any) => (
                  <div key={s.id} className="flex items-center justify-between">
                    <div>
                      <p className="font-medium text-[#03243F] text-sm">{s.trainer_profiles?.full_name}</p>
                      {s.trainer_profiles?.tagline && <p className="text-xs text-gray-500 truncate max-w-[150px]">{s.trainer_profiles.tagline}</p>}
                    </div>
                    <Link href={`/trainers/${s.trainer_profiles?.slug}`} className="text-xs text-[#18A96B] hover:underline">View</Link>
                  </div>
                ))}
              </div>
            )}
