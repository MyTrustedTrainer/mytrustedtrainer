              ...
              <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                {/* Left: logo + optional back link stacked */}
                      <div className="flex flex-col flex-shrink-0">
                                <Link href="/" className="flex items-center gap-2">
                                            <span className="bg-[#18A96B] text-white text-xs font-bold px-2 py-1 rounded">MTT</span>span>
                                            <span className="text-xl font-bold" style={{fontFamily:'Playfair Display'}}>MyTrustedTrainer</span>span>
                                </Link>Link>
                        {showBack && (
                      <button
                                      onClick={() => router.back()}
                ...
                {/* Right: auth section */}
                        ...
                        {/* Avatar — clicking goes directly to profile page */}
                                    <Link
                                      ...
                                      {profile?.avatar_url
                                                          ...
                                                          : <span className="text-white text-sm font-bold">{initials}</span>span>
                                      }
                                    </Link>Link>
                        ...
                        {menuOpen && (
                                        <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50">
                                                          <div className="px-4 py-3 border-b border-gray-100">
                                                                              <p className="text-sm font-semibold text-[#03243F] truncate">{profile?.full_name || user.email}</p>p>
                                                                              <p className="text-xs text-gray-400 truncate">{user.email}</p>p>
                                                            {profile?.plan && (
                                                          ...
                                                          <div className="py-1">
                                                            {profile?.slug && (
                                                                <Link href={'/trainers/' + profile.slug} onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">
                                                                                        <span>&#x1F464;</span>span> View My Profile
                                                                </Link>Link>
                                                                              )}
                                                                              ...
                                                                              <Link href="/for-trainers" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">
                                                                                                    <span>&#x2B06;&#xFE0F;</span>span> Upgrade Plan
                                                                              </Link>Link>
                                                                              <Link href="/dashboard/account" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50">
                                                                                                    <span>&#x2699;&#xFE0F;</span>span> Account Settings
                                                                              </Link>Link>
                                                          </div>div>
                      ...
                      <div className="flex items-center gap-3">
                                    <Link href="/search" className="text-gray-300 hover:text-white text-sm font-medium hidden sm:block">Find Trainers</Link>Link>
                                    <Link href="/for-trainers" className="text-gray-300 hover:text-white text-sm font-medium hidden sm:block">For Trainers</Link>Link>
                                    <Link href="/login" className="text-gray-300 hover:text-white text-sm font-medium border border-gray-500 px-4 py-1.5 rounded-lg hover:border-white">Log In</Link>Link>
                                    <Link href="/signup" className="bg-[#18A96B] text-white text-sm font-semibold px-4 py-1.5 rounded-lg hover:bg-[#15906A]">Sign Up</Link>Link>
                      </div>div>
                                )}
                      </div>div>
              </div>div>
